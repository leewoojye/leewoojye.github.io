...
M2IB는 텍스트 프롬프트 T T와 관련된 부분만 남기고, 나머지 시각 정보는 최대한 버려서 saliency map(이미지에서 어느 부분이 프롬프트와 연관이 높은지 나타내는 맵) $λs$ 를 얻는 것을 목표로 한다.
...
- $λs$ 를 통해 마스킹된 이미지 표현이 텍스트와는 충분한 mutual information을 유지하면서, 원본 이미지와의 불필요한 정보는 줄이도록 하는 M2IB objective를 최적화한다.

참고로 M2IB는 이 둘을 받아서 stochastic mask $λs ∈[0,1]^{H×W}$ 를 출력하는 작은 네트워크(파라미터를 가진)로 구현돼 있다고 한다.